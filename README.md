...
<script>
    const bbsData = [
        { id: 1, name: "1. 坐到站", type: "click", s: ["獨立站起", "手撐獨立站起", "多次嘗試後站起", "少量協助", "大量協助"] },
        { id: 2, name: "2. 獨立站", type: "timer", s: ["穩定 2min", "監督下 2min", "獨立 30s", "嘗試後站 30s", "無法站立 30s"] },
        { id: 3, name: "3. 獨立坐", type: "timer", s: ["穩定坐 2min", "監督下 2min", "坐 30s", "坐 10s", "無法坐 10s"] },
        { id: 4, name: "4. 站到坐", type: "click", s: ["流暢少手撐", "需手控制下降", "腿貼椅下降", "獨立但不穩", "需協助"] },
        { id: 5, name: "5. 床椅轉移", type: "click", s: ["不需手安全轉移", "手輔助安全轉移", "需指導或監督", "一人協助", "二人協助"] },
        { id: 6, name: "6. 閉眼站", type: "timer", s: ["穩定 10s", "監督下 10s", "站立 3s", "閉眼無法維持 3s", "閉眼即跌倒"] },
        { id: 7, name: "7. 併攏站", type: "timer", s: ["獨立 1min", "監督 1min", "獨立 30s", "協助下站 15s", "無法站 15s"] },
        ...
        { id: 10, name: "10. 回頭後看", type: "click", s: ["雙側均可回頭", "僅單側可回頭", "僅轉向側面", "回頭需監督", "需協助"] },
        { id: 11, name: "11. 360轉圈", type: "timer", side: "LR", s: ["雙向 4s 內", "單向 4s 內", "安全但緩慢", "需監督/指導", "需協助"] },
        { id: 12, name: "12. 階梯踏步", type: "timer", s: ["20s 內 8 步", "> 20s 8 步", "監督下 4 步", "協助下 > 2 步", "需協助"] },
        { id: 13, name: "13. 直線站立", type: "timer", foot: "LR", s: ["獨立 30s", "腳稍跨前 30s", "小踏步 30s", "協助下站 15s", "失去平衡"] },
        { id: 14, name: "14. 單腳站立", type: "timer", compare: true, s: ["獨立 > 10s", "5-10s", "3s 以上", "< 3s 但可獨立", "需協助"] }
    ];
    ...
    function showR() {
        let tot = 0; let tb = "";
        bbsData.forEach(item => { const v = currentS[item.id] || 0; tot += v; tb += `<tr><td>${item.id}. ${item.name.split('. ')[1] || item.name}</td><td>${v}</td><td>${currentT[item.id] || ''}</td></tr>`; });
    ...
    function saveAndReset() {
        ...
        const record = { date: new Date().toLocaleString(), id: pId, score: tot };
    ...
    function downloadCSV() {
        ...
        let csvContent = "\uFEFF日期,病歷號,總分\n";
        history.forEach(r => { csvContent += `${r.date},${r.id},${r.score}\n`; });
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.setAttribute("download", `BBS_History_${new Date().getTime()}.csv`);
    ...
    function sendEmail() {
        ...
        const mailtoLink = `mailto:${email}?subject=BBS報告_${pId}&body=${encodeURIComponent(body)}`;
